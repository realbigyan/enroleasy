...
export function TeamPageClient() {
  ...
  async function deleteStaff(id: string, name: string) {
    if (!window.confirm(`Remove ${name} from your organization? This permanently deletes their account and can't be undone.`)) return;
    const res = await fetch(`/api/team/${id}`, { method: "DELETE" });
    if (res.ok) {
      setStaff((prev) => prev.filter((s) => s.id !== id));
    } else {
      const data = await res.json().catch(() => ({}));
      alert(data.error ?? "Could not remove this staff member");
    }
  }
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      {s.role !== "OWNER" && (
                        <>
                          ...
                          <button onClick={() => deleteStaff(s.id, s.name)} title="Delete staff member" className="text-slate-400 hover:text-red-600">
